% --------------------------------------------------------------------
% This MATLAB script simulates a system that consists of a source, and a destination. The source uses a k/n random linear code (RLC) 
% and a 16-QAM modulator. The modulated symbols are transmitted over a Rayleigh fading channel. The destination attempts to recover the
% the sequence of k bits using symbol-level GRAND.
%
% The system is described in  [1] "Symbol-Level GRAND for High-Order Modulation over Block Fading Channels", IEEE Communications Letters, vol. 27, no. 2, Feb. 2023, pp. 447-451.
%  Authors:  Ioannis Chatzigeorgiou, Lancaster University, United Kingdom Francisco Monteiro, Instituto de Telecommunicacoes 
% and ISCTE-Instituto Universitario de Lisboa, Portugal If you use this code or the simulation results, please cite the paper above.
% If you modify this code, please cite the paper above and share your code with the research community.
% --------------------------------------------------------------------
...
k = 103;n = 128;
% Variables for symbol-level GRAND File name of lookup table
sim_name = ['lookup_k',num2str(k),'n',num2str(n),'M16.mat'];
% Maximum weight of error patterns considered,
% error structures that generate error patterns with weight higher than max_weight will not be considered.
max_weight = 3;
% Number of most likely error structures to be considered (all of them will have weight less than or equal to max_weight)  
top_rows   = 5;
% -- END OF INITIALISATION --
% Obtain truncated lookup table based on the full lookup table and parameters max_weight and top_rows
load(sim_name, 'Eb_N0', 'analytical_ordered_error_types')

% Load the full lookup table, not just the entries that correspond to the values of Eb_N0dB
trunc_analytical_ordered_error_types = zeros(top_rows, 3, length(Eb_N0));
% Store the number of error types for each value of Eb/N0 because they might be fewer than top_rows
...
for Eb_N0_idx = 1:length(Eb_N0dB)
    ...
    for rlz_idx = 1:realisations(Eb_N0_idx)
        ...
        % Ensures that if Eb=1 (energy per bit), then Es=4 (energy per symbol), given that 1 symbol carries 4 bits
        s = zeros(1,(1/4)*n);
        ...
        % Additive White Gaussian Noise Both the REAL and the IMAG part of the transmitted_symbols should
        ...
        % Find the appropriate error type / structure for the current value of the fading coefficient
        ...
        fading_specific_ordered_error_types =   trunc_analytical_ordered_error_types(1:trun_error_types_per_entry(Eb_N0_lut_idx), :, Eb_N0_lut_idx);      

        % Symbol-level GRAND
        [est_e, tests, stopped] = symbol_level_GRAND(y_strings, H,  adj_error_patterns, num_of_adj_err_patterns,  fading_specific_ordered_error_types);                
        if stopped == 0
            % Estimated codeword
            est_x = mod(y + est_e, 2);              
            % Error measurement
    ...
    % The values of max_weight and top_rows are included in the name of the data file
...
% The function accepts one input: - input_QAM_array: the array obtained from function build_square_QAM.
% The function returns two look-up tables:
% - get_adj_errors{i}{j}{k}: this is the k-th error pattern of the symbol 
% that is adjacent to the i-th received symbol and can be found in the j-th neighbourhood around the received symbol.
% - get_num_of_adj_errors(i, j): this is size of the j-th neighbourhood of error patterns around the i-th received symbol.
%% This function should be called only once, when the cardinality of the  modulation scheme is selected. do not include it in a loop!
...
%All of of them contain four members or less, depending on the position of the received symbol in the M-QAM constellation)
...
for i = 1:number_of_PAM_symbols
    ...
    for j = 1:number_of_PAM_symbols        
        % convert the value of the binary QAM symbol into an integer and ADD 1 (to avoid having value 0 as an index)
        idx = 0;
        ...
        for hood = 1:2              
            % Count how many neighbours (adjacent symbols)  in the neighbourhood under consideration
            counter = 0; 
            ...
            for member = 1:4              
                ...
                if (new_i >= 1) && (new_i <= number_of_PAM_symbols) && (new_j >= 1) && (new_j <= number_of_PAM_symbols)
                    counter = counter + 1;                    
                    % EITHER activate the line below to store the symbols that are adjacent to the received symbol
                    % adj_symbols{idx}{hood}{counter} = input_QAM_array(:,new_i,new_j);                    
                    % OR ativate the line below to store the error patterns of the symbols that are adjacent to the received symbol
...
% Number of error types to be considered in the algorithm. Change the size of the input argument 'seq_error_types', i.e., 
% increase or decrease the number of rows in 'seq_error_types', to increase or decrease the number of error types.
...
if sum(syndrome)==0 
% The received codeword appears to be a correct codeword, no need to search for an error pattern.
    found=1;
end
current_error_type = 1;
num_of_queries     = 1; % The first query was for the all-zero error pattern
%Testing all the error patterns with a number of errors 1, 2, 3,... up to the defined maximum number of errors :
while (~found) && (current_error_type <= max_num_error_types)    
    outer_test = 1;    
    % Compute all combinations of errors (the number of which is stored in the first column of the appropriate row of 'seq_error_types')
    ...
    % Store all possible combinations for a given value of 'total_symb_errors'.
    num_outer_patterns = size(outer_positions,1);            
    while (~found) && (outer_test <= num_outer_patterns)        
        inner_test = 1;        
        % Identify the received symbols that their positions have been flagged as containing errors
        ...
        while (~found) && (inner_test <= num_inner_patterns)        
            ...
            % Go over the indices that correspond to symbols that will transition to neighbourhood #1
            if symb_errors_in_hood(1) > 0                
                for symb_idx = inner_positions(inner_test, :)                    
                    ...
                    for err_pattern=1:num_of_patterns(lookup_idx, 1)
                        store_adj_errors{symb_idx}{err_pattern} = adjacent_patterns{lookup_idx}{1}{err_pattern}';
                    end                    
                    patterns_per_symbol(symb_idx) =  patterns_per_symbol(symb_idx) + num_of_patterns(lookup_idx, 1);                    
                end                
            end        
            % Go over the remaining indices that correspond to symbols that will transition to neighbourhood #2                      
            if symb_errors_in_hood(2) > 0                
                if symb_errors_in_hood(1) > 0
                    % 'inner_positions' gives the positions of symbols for which neighbourhood #1 will be considered.
                    % 'inner_positions_complement' finds the positions of the  remaining symbols for which neighbourhood #2 will be considered.
                ...
                for symb_idx = inner_positions_complement                    
                    ...
                    for err_pattern=1:num_of_patterns(lookup_idx, 2)
                        store_adj_errors{symb_idx}{err_pattern} = adjacent_patterns{lookup_idx}{2}{err_pattern}';
                    end                    
                    patterns_per_symbol(symb_idx) = patterns_per_symbol(symb_idx) + num_of_patterns(lookup_idx, 2);                    
                end                
            end   
            % Try out all bit error patterns in the consdired neighbourhood of the received symbols for a specific symbol error pattern            
            tested_patterns_per_symbol = ones(1, total_symb_errors);
            % The i-th element in tested_patterns_per_symbol takes values in the range 1, ..., patterns_per_symbol(i).
            ...
            while (~found) && (tested_patterns_per_symbol(1) <= patterns_per_symbol(1))                        
                for symb_idx = 1:total_symb_errors    
                    y_QAM_error_matrix(outer_positions(outer_test, symb_idx), :) =  store_adj_errors{symb_idx}{tested_patterns_per_symbol(symb_idx)};                
                end
...
%In case guessing up to the defined maximum number of errors did not find any valid codeword:
